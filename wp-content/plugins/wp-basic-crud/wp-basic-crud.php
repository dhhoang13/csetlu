...
class Custom_Table_Example_List_Table extends WP_List_Table
 { 
    ...
    function prepare_items()
    {
        ...
        $orderby = (isset($_REQUEST['orderby']) && in_array($_REQUEST['orderby'], array_keys($this->get_sortable_columns()))) ? $_REQUEST['orderby'] : 'code';
...
<tbody >
    ...
    <form >
        <p>			
		    <label for="code"><?php _e('Mã sv:', 'wpbc')?></label>
		<br>	
            <input id="code" name="code" type="text" style="width: 60%" value="<?php echo esc_attr($item['code'])?>"
                    required>
		</p><p>	
            <label for="name"><?php _e('Họ Tên:', 'wpbc')?></label>
		<br>
		...
		<br>
		    ...
		    <label for="class"><?php _e('Lớp:', 'wpbc')?></label> 
		<br>
            <input id="class" name="class" type="text" style="width: 60%" value="<?php echo esc_attr($item['class'])?>" required>
		</p><p>  
...
function wpbc_validate_contact($item)
{
    ...
    if (empty($item['code'])) $messages[] = __('Mã sinh viên là bắt buộc', 'wpbc');
    if (empty($item['name'])) $messages[] = __('Tên là bắt buộc', 'wpbc');
    if (!empty($item['email']) && !is_email($item['email'])) $messages[] = __('Email sai format', 'wpbc');
    if(!empty($item['phone']) && !absint(intval($item['phone'])))  $messages[] = __('Số điện thoại phải lớn hơn 0');
    if(!empty($item['phone']) && !preg_match('/[0-9]+/', $item['phone'])) $messages[] = __('Số điện thoại không thể là chữ');
